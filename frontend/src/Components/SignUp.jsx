...
function SignUp() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const emailExistsResponse = await axios.post('http://localhost:4000/api/check-email', { email: formData.email });
      ...
      if (emailExistsResponse.data.emailExists) {
        ...
        const registrationResponse = await axios.post('http://localhost:4000/api/register', formData);
